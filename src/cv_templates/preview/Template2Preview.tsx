          ...
          <div className="bg-slate-50 h-full p-4 rounded-s-md">
            ...
            <div className="flex flex-col mb-2">
              ...
              <div className="bg-slate-100 p-4 rounded-md">
                <ul className="text-[0.65rem] italic">
                  {contactInfo && (
                    <>
                      <li className="mb-1">{contactInfo.address} ,{contactInfo.location.city}</li>
                      <li className="mb-1">{contactInfo.phone}</li>
                    </>
                  )}

                  {socialLinks && socialLinks.map((social, index) => (
                    <li key={index} className="mb-2"><FontAwesomeIcon icon={iconNames[social.platform]} /> <a className="italic hover:underline ml-1 " href={social.link} target="_blank"> {social.link}</a> </li>
                  ))}
            ...
            <div className="flex flex-col">
              ...
              <div className="bg-slate-100 p-3 rounded-md ">
                {refree && refree.map((ref, index) => (
                  <div className="flex flex-col mb-2" key={index}>
                    <h4 className="font-medium capitalize">{ref.refreeName}</h4>
                    <span className="text-sm capitalize ">{ref.positions}</span>
                    <span className="text-xs ">{ref.institute} </span>
                    <span className="text-xs italic">{ref.location?.city}, {ref.location?.state}</span>
              ...
              <div className="bg-slate-50 p-4 rounded-md ">
                {experience && experience.map((exp, index) => (
                  <div className="flex flex-col mb-2" key={index}>
                    ...
                    <span className="text-[0.65rem]">{formattedDate(exp.dates)}</span>
                    <span className="text-[0.65rem]">{exp.location?.city} ,{exp.location?.state}</span>
              ...
              <div className="bg-slate-50 p-4 rounded-md ">
                {educations && educations.map((edu, index) => (
                  <div className="flex flex-col mb-2" key={index}>
                    ...
                    <span className="text-[0.65rem]">{formattedDate(edu.dates)}</span>
                    <span className="text-[0.65rem]">{edu.location?.city},{edu.location?.state}</span>
                    ...
                    <div className="flex flex-col mb-2" key={index}>
                      ...
                      <ul className="flex text-[.65rem] gap-3 ">
                        {custom.urls.map((url, index) => (
                          <li key={index} className="mb-2"><FontAwesomeIcon icon={iconNames[url.platform]} /> <a className="italic hover:underline ml-1 " href={url.link} target="_blank"> {url.link}</a> </li>
                        ))}
                      </ul>
