...
import { faArrowCircleRight, faArrowLeft, faArrowRight, faPen, faPlusCircle, faRepeat } from '@fortawesome/free-solid-svg-icons';
...
const InputHighlight = ({ templateId }: { templateId: number }) => {
    ...
    const { heading, editingHighlight, editingHighlightDesc, editingHighlightUrl }: {
        heading: string,
        ...
        <motion.div initial={{ opacity: 0, x: -50 }}
            ...
            <form onSubmit={handleSubmit}>
                <div className='flex flex-col gap-3'>
                    ...
                    <div>
                        {editMode && editingHighlight?.urls &&
                            editingHighlight.urls.length > 0 ? (
                            <ul className="text-xs text-slate-900">
                                {editingHighlight.urls.map((url, index) => (
                                    <li key={index} className="relative">
                                        {editMode &&
                                            <button type='button' onClick={() => dispatch(editHighlightUrl(index))}>
                                                <FontAwesomeIcon
                                                    icon={faPen} size="lg"
                    ...
                    <div className='flex w-full justify-center items-center gap-3 max-lg:flex-wrap'>
                        ...
                        selectedKeys={new Set (editingHighlightUrl?.platform ? [editingHighlightUrl.platform]:[] )}
                        ...
                        <Input
                            ...
                            endContent={<Button isIconOnly onPress={handleUrls} >
                                <FontAwesomeIcon size="lg" icon={editMode ? faRepeat : faArrowCircleRight} />
                            </Button>}
